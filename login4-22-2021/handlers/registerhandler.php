...
$reg_check_query="SELECT * FROM `Authors` WHERE `AuthorUserName` = '$userName' AND `AuthorEmail` = '$email' LIMIT 1"; 

$result = mysqli_query($db, $reg_check_query); 
$user = mysqli_fetch_assoc($result); 

if($user){
    if($user['AuthorUserName'] == $userName){ 
        array_push($Reg_error, "That User Name has already been chosen"); 
        
    }
    
    if($user['AuthorEmail'] == $email){ 
...
if(count($Reg_error) == 0){ 
        ...
        $query = "INSERT INTO `Authors` (`AuthorUserName`, `AuthorEmail`, `Password`) VALUES ('$userName', '$email', '$safe_password')";
